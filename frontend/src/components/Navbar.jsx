...
const Navbar = () => {
  ...
  return (
    <div className="flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-300">
      <img onClick={()=>navigate('/')} className="w-60 cursor-pointer" src={assets.logof} alt="" />
  ...
  <hr className="border-none outline-none bg-[#5f6FFF] w-3/5 m-auto h-0.5 hidden" />
      ...
      <div className="flex items-center gap-4">
        ...
        token ? (
          <div className="flex items-center gap-2 cursor-pointer group relative">
            <img className="w-8 rounded-full" src={assets.profile_rahim} alt="" />
      ...
      <div className={` ${showMenu ? 'fixed w-full' :'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
        <div className="flex items-center justify-between px-5 py-6">
          <img className="w-80" src={assets.logof} alt="" />
